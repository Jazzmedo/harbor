...
import { episodeSpanContains, episodeSpanLabel, parseEpisodeSpan } from "../src/lib/episode-span.ts";
...
for (const filename of ["Show.S01E01E02.mkv", "Show S1E1-E2.mp4", "Show.S01E01-02.mkv", "Show.1x01-02.mkv"]) {
...
test("single episodes use the same start and end", () => {
  assert.deepEqual(parseEpisodeSpan("Show.S02E007.1080p.mkv"), { season: 2, episode: 7, episodeEnd: 7 });
});
...
test("rejects reversed, non-consecutive, cross-season, and bare-number ranges", () => {
  for (const filename of ["S01E02-E01.mkv", "S01E01-E03.mkv", "S01E01-S02E02.mkv", "Show.01-02.1080p.mkv", "Show.2025-2026.mkv"]) {
...
test("containment and file selection include either covered episode", () => {
  ...
  assert.equal(matchEpisodeFileIndex(["Show.S01E01-E02.mkv", "Show.S01E03.mkv"], { season: 1, episode: 2 }), 0);
