...
function taskbarError() {
  if (!isNative()) return;
  try { T.window.getCurrentWindow().setProgressBar({ status: 'error' }); } catch (e) {}
}
...
export async function fetchChangelog() {
  if (!isNative()) {
    const res = await fetch('https://harbor.site/updates/versions-beta.json', { cache: 'no-store' });
...
export async function minimizeWindow() {
  if (!isNative()) return;
  try { await T.window.getCurrentWindow().minimize(); } catch (e) {}
}
...
export async function isMaximized() {
  if (!isNative()) return false;
  try { return await T.window.getCurrentWindow().isMaximized(); } catch (e) { return false; }
}

export async function onWindowResized(handler) {
  if (!isNative()) return () => {};
  try { return await T.window.getCurrentWindow().onResized(handler); } catch (e) { return () => {}; }
}
