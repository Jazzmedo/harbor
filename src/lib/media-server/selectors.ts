import type { MediaIdentity, MediaServerConnection, MediaServerItem, MediaServerTitle, PlayableCopy } from "./types";
...
export function identityKey(identity: MediaIdentity, kind: "movie" | "series"): string | null {
  if (identity.tmdbId != null) return `tmdb:${kind === "movie" ? "movie" : "tv"}:${identity.tmdbId}`;
...
export function groupMediaServerTitles(items: MediaServerItem[]): MediaServerTitle[] {
  ...
  const seriesByConnectionAndId = new Map(items.filter((item) => item.kind === "series").map((item) => [`${item.connectionId}:${item.id}`, item]));
  for (const item of items) {
    const kind = baseKind(item);
    if (!kind || item.kind === "season") continue;
    const parent = item.kind === "episode" && item.parentId ? seriesByConnectionAndId.get(`${item.connectionId}:${item.parentId}`) : undefined;
    ...
    if (!group) {
      group = { title: { key, kind, identity: { ...groupIdentity, season: undefined, episode: undefined }, fallbackTitle: groupTitle, year: groupYear, itemKeys: [], connectionIds: [], libraryIds: [], episodeCount: 0, versionCount: 0, addedAt: item.addedAt, progress: item.progress }, physical: new Set() };
      groups.set(key, group);
    }
    const itemKey = `${item.connectionId}:${item.id}`;
    group.title.itemKeys.push(itemKey);
    if (!group.title.connectionIds.includes(item.connectionId)) group.title.connectionIds.push(item.connectionId);
    if (!group.title.libraryIds.includes(item.libraryId)) group.title.libraryIds.push(item.libraryId);
    ...
    if ((item.progress?.updatedAt ?? 0) > (group.title.progress?.updatedAt ?? 0)) group.title.progress = item.progress;
  }
  return [...groups.values()].map((group) => group.title);
}

export function matchingServerItems(items: MediaServerItem[], identity: MediaIdentity, _kind: "movie" | "series", season?: number, episode?: number): MediaServerItem[] {
  const seriesById = new Map(items.filter((item) => item.kind === "series").map((item) => [`${item.connectionId}:${item.id}`, item]));
  const candidates = items.filter((item) => {
    const parent = item.kind === "episode" && item.parentId
  ...
  const exact = candidates.filter((item) => item.identity.season === season && item.identity.episode === episode);
  ...
  const selected = candidates.filter((item) => {
    ...
    return episodeSpanContains(item.identity, season, episode) ||
  ...
  return selected.flatMap((item) => {
    ...
    const versions = item.versions.filter((version) => {
      ...
      return episodeSpanContains(version, season, episode) ||
...
export function matchingServerEpisodes(items: MediaServerItem[], identity: MediaIdentity): MediaServerItem[] {
  const seriesById = new Map(items.filter((item) => item.kind === "series").map((item) => [`${item.connectionId}:${item.id}`, item]));
...
export function dedupePhysicalEpisodeItems(items: MediaServerItem[]): MediaServerItem[] {
  ...
  for (const item of items) {
    ...
    const score = spanSeason != null && spanEpisode != null &&
      item.identity.season === spanSeason && item.identity.episode === spanEpisode
      ? 1
      : 0;
...
export function serverPlayableCopies(items: MediaServerItem[], connections: MediaServerConnection[]): PlayableCopy[] {
  ...
  return items.flatMap((item) => {
    ...
    return item.versions.flatMap((version) => {
      ...
      seen.add(physical);
      return [{ key: `${item.connectionId}:${item.id}:${version.id}`, category: "home-server" as const, label: version.name ?? item.title, sourceLabel: connection.name, version, progress: item.progress, connectionId: item.connectionId, itemId: item.id }];
    });
  });
}
