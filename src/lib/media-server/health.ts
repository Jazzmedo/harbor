...
export function getMediaServerHealthSnapshot() { return snapshot; }
export function subscribeMediaServerHealth(listener: () => void) { listeners.add(listener); return () => listeners.delete(listener); }

export function markMediaServerInactive(connectionId: string) {
  update(connectionId, "inactive");
}

export function probeMediaServerHealth(connection: MediaServerConnection): Promise<MediaServerHealth> {
  if (!connection.enabled) { update(connection.id, "inactive"); return Promise.resolve("inactive"); }
  const existing = pending.get(connection.id);
  if (existing) return existing;
  if (!snapshot[connection.id]) update(connection.id, "checking");
  const request = mediaServerAdapter(connection).libraries(connection)
    .then(() => { update(connection.id, "active"); return "active" as const; })
    .catch(() => { update(connection.id, "inactive"); return "inactive" as const; })
