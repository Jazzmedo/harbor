...
function write(value: MediaServerConnection[]) { localStorage.setItem(KEY, JSON.stringify(value)); listeners.forEach((fn) => fn()); }
export function mediaServerConnections(profileId = activeProfileId()) { return all().filter((c) => c.profileId === profileId); }
...
export function updateMediaServerConnection(id: string, patch: Partial<Omit<MediaServerConnection, "id" | "profileId" | "provider">>, profileId = activeProfileId()) {
  const current = all().find((connection) => connection.id === id && connection.profileId === profileId);
  if (!current) return;
...
export function mediaServerToken(connection: MediaServerConnection) { return getSecret(`${TOKEN}.${connection.profileId}.${connection.id}`); }
export function subscribeMediaServerConnections(fn: () => void) { listeners.add(fn); return () => { listeners.delete(fn); }; }
...
export function mediaServerSyncDue(connection: MediaServerConnection, now = Date.now()): boolean {
  ...
  const interval = connection.refreshInterval === "custom"
