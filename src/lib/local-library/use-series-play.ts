...
export function useLocalAwareSeriesPlay() {
  ...
  return useCallback(
    async (args: {
      ...
      const serverItems = matchingServerItems(indexedItems, identity, "series", episode.season, episode.episode);
      ...
      if (decision.kind === "local" && versions[0]) { openPlayer(localPlayerSrc(versions[0], srcIsAnime, episode)); return; }
      if (decision.kind === "home-server") {
        const item = serverItems.find((candidate) => candidate.connectionId === decision.copy.connectionId && candidate.id === decision.copy.itemId);
        const connection = item && connections.find((candidate) => candidate.id === item.connectionId);
        if (item && connection) openPlayer(await createMediaServerPlayerSrc({ meta, imdbId: seriesImdb ?? undefined, episode, connection, item, versionId: decision.copy.version.id }));
        return;
      }
      openLocalVersions({
        title: meta.name,
        ...
        onPlayServer: async (copy) => {
          ...
          const connection = item && connections.find((candidate) => candidate.id === item.connectionId);
          if (item && connection) openPlayer(await createMediaServerPlayerSrc({ meta, imdbId: seriesImdb ?? undefined, episode, connection, item, versionId: copy.version.id }));
        },
        onStream: stream,
      });
    },
