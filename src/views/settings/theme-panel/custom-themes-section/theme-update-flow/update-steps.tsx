...
export function PickThemeStep({
  ...
  if (themes.length === 0) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-2 rounded-md border border-dashed border-edge px-6 py-16 text-center">
        <span className="text-[15px] font-semibold text-ink">{t("No local themes to publish")}</span>
        <span className="max-w-[38ch] text-[13px] text-ink-muted">
          {t("Build or import the updated theme first, then come back to push it as a new version.")}
        </span>
      </div>
    );
  }
  return (
    <div className="flex flex-col gap-4">
      <p className="text-[13.5px] text-ink-muted">{t("Pick the theme with your latest changes. It becomes the new version.")}</p>
...
export function ShotsStep({ shots, onAdd, onRemove }: { shots: { url: string }[]; onAdd: () => void; onRemove: (i: number) => void }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <p className="text-[13.5px] text-ink-muted">{t("Optional. Adding any screenshots replaces your current set.")}</p>
...
export function ChangelogStep({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  return (
    <div className="flex max-w-[460px] flex-col gap-5">
      <p className="text-[13.5px] text-ink-muted">{t("Tell people what changed in this version. Reviewers see it too.")}</p>
    ...
    <div className="flex flex-1 flex-col items-center justify-center gap-5 px-10 text-center">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <p className="max-w-[42ch] text-[13.5px] text-ink-muted">
          {t("Your new version is in for a quick review. The live listing keeps working until this one is approved.")}
        </p>
      </div>
