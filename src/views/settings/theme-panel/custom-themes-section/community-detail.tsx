...
export function CommunityDetail({
  ...
  return createPortal(
    <div className={`${closing ? "animate-scrim-out" : "animate-scrim-in"} fixed inset-0 z-[244] flex items-center justify-center p-6`}>
      <button aria-label={tr("Close")} onClick={close} className="absolute inset-0 cursor-default bg-canvas/70 backdrop-blur-sm" />
      <div className={`modal-panel ${closing ? "animate-dialog-out" : "animate-dialog-in"} relative flex max-h-[88vh] w-full max-w-2xl flex-col overflow-hidden rounded-md bg-elevated harbor-float`}>
        <button aria-label={tr("Close")} onClick={close} className="absolute end-3 top-3 z-10 flex h-8 w-8 items-center justify-center rounded-full bg-canvas/80 text-ink-muted backdrop-blur-md transition-colors hover:text-ink">
          <X size={16} />
        </button>
          ...
          <div className="flex flex-col gap-4 p-6">
            <div>
              ...
              <p className="text-[13px] text-ink-subtle">
                {tr("by {author}", { author: t.author })} · {t.downloads === 1 ? tr("1 download") : tr("{count} downloads", { count: t.downloads })} · {tr("{rating}/5 ({count})", { rating: t.ratingAvg || "-", count: t.ratingCount })}
              </p>
            </div>
 ...
 className="flex h-11 items-center gap-2 rounded-md px-4 text-[13.5px] font-medium text-ink-muted transition-colors hover:bg-canvas hover:text-ink"
              >
                {copied ? <Check size={16} /> : <Share2 size={16} />} {copied ? tr("Copied") : tr("Share")}
              </button>
              <div className="ms-auto flex items-center gap-0.5" role="group" aria-label={tr("Rate this theme")}>
                {[1, 2, 3, 4, 5].map((n) => (
                  <button key={n} onClick={() => rate(n)} aria-label={tr("Rate {count} stars", { count: n })} className="p-0.5">
