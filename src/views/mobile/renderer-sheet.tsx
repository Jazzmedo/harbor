...
export function RendererSheet({ open, onClose, title = "Play on" }: { open: boolean; onClose: () => void; title?: string }) {
  ...
  const localName = hostLabel(target.kind === "local" ? target.label : undefined) ?? t("This Harbor");
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 pb-2 pt-4">
          ...
          {snapshot.castDiscovering && <span className="text-[12px] text-ink-subtle">{t("Scanning…")}</span>}
        </div>
